...
namespace {
enum Ink : unsigned { Original, Track, White, Cyan, Selection, Blue, Yellow, Green, Red, Amber, InkCount };
...
uint32_t model_ink(uint32_t descriptor) {
    descriptor &= 0xfffff;
    // The opening tunnel end and side sections use the model-copy path too.
    if (descriptor == 0x61080 + 34 * 16 || descriptor == 0x61080 + 35 * 16
        || descriptor == 0x61080 + 59 * 16) return Track;
    if (descriptor < 0x62080 || descriptor >= 0x62100 || (descriptor & 15)) return Original;
    // Model pairs 256/257, 258/259, 260/261, 262/263 share vertices.
    // Menu order: Falcon (258/259), 260/261, 262/263, 256/257.
    static const unsigned inks[] = {Red, Red, Blue, Blue, Yellow, Yellow, Green, Green};
    return inks[(descriptor - 0x62080) / 16];
}
...
uint32_t observe_store(const CPUState* cpu, uint32_t address, uint32_t, unsigned width) {
    ...
    if (((phys >> 24) & 7) == 5) {
        ...
        if (off >= 0x4220 && off < 0x5230 && ((off - 0x4220) & 7) == 0 && width == 2) {
            ...
            if (pc == 0x1403c) edge_tags[(off - 0x4220) / 8] = model_ink(cpu->gpr[6]);
...
void render(const VbRenderFrame* frame, uint32_t* out, void*) {
    ...
    for (unsigned i = 0; i < VB_RENDER_PIXELS; ++i) {
        ...
        unsigned ink = Original;
        if (source.kind == 4) ink = source.tile_hash;
            ...
            if (found != materials.end()) ink = found->second[source.v * 8 + source.u];
        }
    ...
    if (vb_mod_asset("materials.txt", path, sizeof(path))) {
        ...
        while (input >> std::dec >> map >> kind >> x >> y >> std::hex >> hash >> mask) {
            ...
            for (unsigned i = 0; i < 64; ++i) {
                if (mask[i] < '0' || mask[i] >= '0' + int(InkCount)) {valid = false; break;}
                values[i] = uint8_t(mask[i] - '0');
            }
